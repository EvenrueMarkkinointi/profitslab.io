...
import { Check, Circle, Triangle, ArrowRight } from 'lucide-react';
...
export default function WhatYouGetSection({ className = '' }: Props) {
  ...
  return (
    ...
    >
      ...
      <div 
        ref={triangleRef}
        className="absolute left-[46vw] top-[70vh] w-[5vw] max-w-[70px] z-20"
      >
      ...
      >
        <div>
          <h2 className="font-display text-[clamp(28px,3.5vw,48px)] text-[#F6F3EA] uppercase leading-[0.95] mb-6">
            A READY‑TO‑RUN PROMPT SYSTEM.
          </h2>
          
          <p className="text-[#D7D2F0] text-[clamp(14px,1.1vw,17px)] leading-relaxed mb-8">
            80 battle-tested prompts across 10 business jobs. Copy, paste, customize the brackets—get usable output in minutes.
          </p>
        ...
        <button 
          onClick={() => document.getElementById('inside-stack')?.scrollIntoView({ behavior: 'smooth' })}
          className="flex items-center gap-2 text-[#E46B67] hover:text-[#F6F3EA] transition-colors group"
        >
          <span className="font-mono-label text-[11px]">See what's inside</span>
